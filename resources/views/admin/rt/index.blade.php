...
@section('page_title', 'Master Data RT & Petugas Wilayah')
...
<div class="space-y-6" x-data="{ createModal: false, editModal: false, editRt: {} }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 sm:p-6 bg-white rounded-2xl border-2 border-slate-200 shadow-md">
        <div>
            <h3 class="text-base sm:text-lg font-black text-slate-900">Daftar Wilayah RT &amp; Petugas Penanggung Jawab</h3>
            <p class="text-xs text-slate-500 mt-1 font-medium">Setiap Petugas bertindak sebagai RT yang mengurus unit RW / Warga binaannya masing-masing.</p>
        </div>

        <button @click="createModal = true" 
                class="inline-flex items-center gap-2 px-5 py-2.5 text-xs font-black text-white bg-sky-600 hover:bg-sky-500 rounded-xl shadow-md transition">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        @foreach($rts as $rt)
            <div class="bg-white rounded-2xl border-2 border-slate-200 shadow-md hover:shadow-xl hover:border-sky-300 transition p-5 flex flex-col justify-between group">
                <div>
                    <div class="flex items-start justify-between">
                        <div>
                            ...
                            <h4 class="text-lg font-black text-slate-900 mt-1.5">{{ $rt->nama_rt }}</h4>
                            <p class="text-xs text-slate-500 font-medium">{{ $rt->wilayah }}</p>
                        </div>

                        <!-- Action Buttons -->
                        <div class="flex items-center gap-1">
                            <button @click="editRt = {{ $rt->toJson() }}; editModal = true"
                                    class="p-2 text-slate-400 hover:text-sky-600 hover:bg-sky-50 rounded-xl transition border border-transparent hover:border-sky-200" title="Edit RT">
                            ...
                            <form action="{{ route('admin.rt.destroy', $rt->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus wilayah {{ $rt->nama_rt }}?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition border border-transparent hover:border-rose-200" title="Hapus RT">
                    ...
                    @if($rt->keterangan)
                        <p class="text-xs text-slate-700 bg-slate-50 p-3 rounded-xl mt-3 border border-slate-200 italic font-medium">
                            "{{ $rt->keterangan }}"
                        </p>
                    @endif
                    ...
                    <div class="mt-4 pt-3 border-t-2 border-slate-100">
                        ...
                        <div class="mt-2 space-y-2">
                            @forelse($rt->petugas as $p)
                                <div class="flex items-center gap-2.5 text-xs font-bold text-slate-800 bg-slate-50 p-2 rounded-xl border border-slate-200">
                                    <div class="w-7 h-7 rounded-lg bg-teal-600 text-white flex items-center justify-center text-xs font-black shadow-sm">
                                        {{ strtoupper(substr($p->name, 0, 1)) }}
                                    </div>
                                    <div class="min-w-0 flex-1">
                                        <p class="truncate">{{ $p->name }}</p>
                                        <p class="text-[10px] text-slate-400 font-normal truncate">{{ $p->phone ?? $p->email }}</p>
                                    </div>
                                </div>
                            @empty
                                <p class="text-xs text-amber-700 bg-amber-50 p-2 rounded-xl border border-amber-200 font-bold">Belum ada petugas RT yang ditugaskan</p>
                            @endforelse
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-5 pt-3.5 border-t-2 border-slate-200 flex items-center justify-between text-xs">
                    <span class="text-slate-600 font-bold bg-slate-100 px-2.5 py-1 rounded-lg border border-slate-200">{{ $rt->pelanggans_count }} RW / Warga Binaan</span>
    ...
    <div x-show="createModal" class="fixed inset-0 z-50 overflow-y-auto" x-cloak>
        <div class="flex items-center justify-center min-h-screen p-4 text-center">
            ...
            <div class="inline-block bg-white rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:max-w-lg sm:w-full border-2 border-slate-300">
                <div class="bg-gradient-to-r from-sky-700 to-teal-700 px-6 py-4 text-white flex items-center justify-between">
                ...
                <form action="{{ route('admin.rt.store') }}" method="POST" class="p-6 space-y-4">
                    @csrf
                    <div>
                        <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Kode RT</label>
                        <input type="text" name="kode_rt" required placeholder="Contoh: RT-01" class="w-full px-4 py-2.5 border-2 border-slate-300 rounded-xl text-sm font-bold focus:ring-2 focus:ring-sky-500 focus:outline-none">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Nama RT / RW</label>
                        <input type="text" name="nama_rt" required placeholder="Contoh: RT 01 / RW 01" class="w-full px-4 py-2.5 border-2 border-slate-300 rounded-xl text-sm font-bold focus:ring-2 focus:ring-sky-500 focus:outline-none">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Nama Dusun</label>
                        <input type="text" name="wilayah" required value="Dusun Argosari" placeholder="Contoh: Dusun Argosari" class="w-full px-4 py-2.5 border-2 border-slate-300 rounded-xl text-sm font-medium focus:ring-2 focus:ring-sky-500 focus:outline-none">
                    </div>
                    <div>
                        ...
                        <p class="text-[11px] text-slate-500 mt-1 font-medium">Tahan tombol Ctrl (Windows) untuk memilih petugas RT.</p>
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Keterangan Wilayah</label>
                        <textarea name="keterangan" rows="2" placeholder="Catatan wilayah Dusun, RT, dan RW..." class="w-full px-4 py-2 border-2 border-slate-300 rounded-xl text-sm font-medium focus:ring-2 focus:ring-sky-500 focus:outline-none"></textarea>
                    </div>
    ...
    <div x-show="editModal" class="fixed inset-0 z-50 overflow-y-auto" x-cloak>
        <div class="flex items-center justify-center min-h-screen p-4 text-center">
            ...
            <div class="inline-block bg-white rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:max-w-lg sm:w-full border-2 border-slate-300">
                ...
                <form :action="'/admin/rt/' + editRt.id" method="POST" class="p-6 space-y-4">
                    @csrf
                    @method('PUT')
                    <div>
                        <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Kode RT</label>
