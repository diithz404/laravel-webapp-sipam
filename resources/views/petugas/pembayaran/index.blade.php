    ...
    <div class="space-y-3">
        ...
        <div class="bg-white rounded-2xl border-2 transition shadow-sm p-4 sm:p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 {{ $isLunas ? 'border-emerald-200 bg-emerald-50/15' : ($isSebagian ? 'border-amber-200 bg-amber-50/15' : 'border-slate-200 hover:border-slate-300') }}"
            ...
            <div class="space-y-1.5 flex-1 min-w-0">
                ...
                <p class="text-xs text-slate-500 truncate">{{ $catatan->pelanggan->alamat }}</p>
            ...
            <div class="flex items-center gap-2 border-t md:border-t-0 pt-3 md:pt-0 border-slate-100 shrink-0">
                <div class="flex items-center gap-2 w-full md:w-auto">
                    @if(!$isLunas)
                        ...
                        @endif
                    @endif
                    <a href="{{ $waCardUrl }}" target="_blank" class="p-2.5 bg-emerald-50 border border-emerald-200 rounded-xl text-emerald-700 hover:bg-emerald-100 transition">
                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414zM12 0C5.373 0 0 5.373 0 12c0 2.106.551 4.168 1.597 5.975L0 24l6.192-1.618A11.934 11.934 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0z"/></svg>
                    </a>
                    <a href="{{ route('kwitansi.show', $catatan->id) }}" target="_blank" class="p-2.5 bg-sky-50 border border-sky-200 rounded-xl text-sky-800 hover:bg-sky-100 transition">
